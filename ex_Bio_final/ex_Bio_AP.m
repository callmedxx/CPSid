...
% A1 = [exp((tmp+80)/(-6.8)) exp((tmp+80)/(-6.8)).*x(:,3) (global_par_beta_h1).*x(:,3)  global_par_beta_h2.*x(:,3)];
% dictionary matrix of gating variable j 
A1 = [global_par_alpha_j1 global_par_alpha_j1.*x(:,4)  (exp((-0.01052)*tmp)./(1+exp((-0.1378)*(tmp+40.14)))).*x(:,4)  (exp((-2.535E-7)*tmp)./(1+exp((-0.1)*(tmp+32)))).*x(:,4)];
state = zeros(size(y4));
state((40+tmp) < 0) = 1;
state((40+tmp) > 0) = 2;
